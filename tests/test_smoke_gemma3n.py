...
def test_text_completion_smoke_two_steps(tmp_path, monkeypatch):
    """Uses completion mode so the tiny HF stub does not require a chat template."""
    ...
    data_dir = tmp_path / "data" / "datasets" / "text-smoke"
...
"""
    ...
    try:
        ...
        cfg.read("config.ini")
        profile = load_profile_config(cfg, "text-smoke")
        out = tmp_path / "out_smoke_text"
